...
const cvFormatter = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr[i].yearsOfExperience > 1) {
      ...
      newApplicantsArray.push(newData);
    }
  }
  return newApplicantsArray;
};
// -------------------------------------------------------------------------------------------------------
...
const classesAvg = (data) => {
  for (let i = 0; i < data.grades.length; i++) {
    for (let j = 0; j < data.grades[i].classes.length; j++) {
      let classScorestotal = 0;
      let scoresNumber = 0;
      for (let k = 0; k < data.grades[i].classes[j].classScores.length; k++) {
        classScorestotal += data.grades[i].classes[j].classScores[k];
        scoresNumber++;
      }
      let newAvg = Math.floor(classScorestotal / scoresNumber);
