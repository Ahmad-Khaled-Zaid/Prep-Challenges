...
const recursionPattern = (int1, int2) => {
  ...
  const recursionPattern2 = (int1, int2) => {
    if (array.length == 0) {
      array.push(int1);
    }
    if (
      (array[array.length - 1] == null || array[array.length - 1] > 0) &&
      flag == true
    ) {
      array.push(int1 - int2);
      positiveLength = array.length;
      recursionPattern2(array[array.length - 1], int2);
    } else {
      flag = false;
      if (positiveLength > 1) {
...
const filterLinks = (str) => {
  ...
  return str.slice(linkStart, linkEnd + 4);
};
// -------------------------------------------------------------------------------------------------------
